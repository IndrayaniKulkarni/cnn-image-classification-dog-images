...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    """  
    #results_dic contains filename , pet label and classifier label
    dognames_dic = {} #dog names is a dictionary with the key as the dog name and a value of 1 (arbitrary value). 
    f = open(dogfile, 'r')
   
    all_lines = f.readlines()
    for line in all_lines:
      # dogname = line.rstrip()
      dognames = line.lower().strip().split(',')
    ...
    for filename in results_dic:
      ...
      for label in classifier_labels:
        if label in dognames_dic: 
          ...
          classifier_match = 0
      results_dic[filename].extend([pet_match, classifier_match])
